...
\framebox[0.95\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} CALC & si ~s=QUERY \wedge  disponibles\_i=P\\
		\end{cases}$\\
		
		$\lambda:~\lambda(S)\begin{cases}
		query\_o = N  & si ~s=IDLE\\
		pedido\_o = P & si ~s=CALC\\
		\end{cases}$\\
...
\framebox[0.95\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} ACCEPT & si ~s=QUERY \wedge  disponibles\_i=N\\
		DECLINE & si ~s=QUERY \wedge disponibles\_i < N
		\end{cases}$\\
		
		$\lambda:~\lambda(S)\begin{cases}
		query\_o = N &si~s=IDLE\\
		pedido\_o = 0 &si ~s=DECLINE\\
		pedido\_o = N &si ~s=ACCEPT
		\end{cases}$\\
...
\begin{minipage}{0.9\textwidth}
	...
	$\delta_{ext}:\delta_{ext}(S,numProd\_i) \begin{cases} CALC & si ~s=QUERY \wedge  numProd\_i=N
	\end{cases}$\\
	
	$\lambda:~\lambda(S)\begin{cases}
	queryInventory\_o = 1 & si ~s=WAITING\\
	querySuppliers\_o = S-N & si ~s=CALC \wedge numProd\_i < n \\
	querySuppliera\_o = 0 & si ~s=CALC \wedge numProd\_i \geq n
	\end{cases}$\\
...
En el pseudocódigo se puede observar la política de reposición de insumos explicada en la sección~\ref{sec:CID}. En la función de salida $\lambda$ se implementa la Ecuación~\eqref{eq:ctrlinv} donde:\\
$S = $ es la capacidad de almacenamiento del inventario.\\
$n = $ es el parámetro de política de reposición.
...
\framebox[0.95\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\delta_{int}: \delta_{int}(S) \begin{cases}
		INV\_WAIT & si ~s=QUERY\\
		QUERY & si ~s=CHECK \wedge (numPassProd <\\
		& \hfill numClientQuery)~\wedge\\
		& \hfill (\neg{invEmpty})\\
		SEND &si ~s=CHECK \wedge \forall~otra~condicion.\\
		WAITING &si ~s=SEND
		\end{cases}$\\
		
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} QUERY &si ~s=WAITING \wedge  queryClient\_i=N\\
		SEND &si ~s=CHECK \wedge size(prod\_i)=P
		\end{cases}$\\
		
		$\lambda:~\lambda(S)\begin{cases}
		queryInventory\_o = 1 & si ~s=QUERY\\
		\O & si ~s=CHECK\\
		prod\_o = K & si ~s=SEND
		\end{cases}$\\
...
\framebox[0.95\textwidth]{
	\begin{minipage}{0.94\textwidth}%
		...
		$\delta_{int}: \delta_{int}(S) \begin{cases}
		INV\_WAIT & si ~s = QUERY\\
		CLI\_WAIT & si ~s=CLI\_RPLY\\
		WAITING & si ~s=INV\_GET
		\end{cases}$\\
		
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} 
		QUERY & si ~s=WAITING \wedge  queryClient\_i=N\\
		CLI\_RPLY & si ~s=INV\_WAIT \\
		& \hfill \wedge ~queryInventory\_i=P\\
		INV\_GET & si ~s=CLI\_WAIT \\
		& \hfill \wedge ~ numProdClient\_i=K\\
		\end{cases}$\\
		
		$\lambda:~\lambda(S)\begin{cases}
		queryInventory\_o = N & si ~s = QUERY\\
		queryClient\_o = P & si ~ s=CLI\_RPLY\\
		prodInventory\_o = K & si ~s=INV\_GET
		\end{cases}$\\
...
\framebox[0.95\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} QUERY & si ~s=IDLE \wedge \begin{cases} product\_in\_A = Na \\ product\_in\_B = Nb \\ product\_in\_C = Nc \\ \end{cases}\\
		& \hfill \wedge ~\exists i \in \{A,B,C\}: ~size(cola\_i) == 0\\
		ENCARGO & si ~s= IDLE \wedge encargo = Ne\\
		PUSH & si ~s=IDLE \wedge \begin{cases} product\_in\_A = Na \\ product\_in\_B = Nb \\ product\_in\_C = Nc \\ \end{cases}\\
		& \hfill \wedge ~\forall i \in \{A,B,C\}: ~size(cola\_i) != 0 \\
		GET & si ~s=IDLE \wedge get = Ng\\
		\end{cases}$\\
		
		$\lambda:~\lambda(S)\begin{cases}
		query\_out = size(cola) - encargos\_q & si ~s = QUERY\\
		\O & si~s=ENCARGO\\
		producto\_out = \min(get\_q,size(cola)) & si ~s = PUSH\\
		producto\_out = \min(encargos\_q,size(cola)) & si~s = GET\\
		\end{cases}$\\
...
\framebox[1\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\delta_{int}: ~\delta_{int}(S) \begin{cases} IDLE & si~s=SERVE \end{cases}$\\
		
		$\delta_{ext}:\delta_{ext}(S,X) \begin{cases} SERVE & si ~s=IDLE \wedge  pedido = N\\
		\end{cases}$\\
		
		$\lambda:~\lambda(S) \begin{cases} entrega = N & si~s = SERVE \end{cases}$\\
...
\framebox[1\textwidth]{
	\begin{minipage}{0.9\textwidth}
		...
		$\lambda:~\lambda(S) \begin{cases} 
		entrega = N & si ~s = SERVE \\
		& \hfill \wedge ~N\%numProdPkg = 0 \\
		entrega = \lfloor N/numProdPkg \rfloor \\
		\hfill + N\%numProdPkg & si ~s = SERVE \wedge ~\forall~N 
		\end{cases}$\\
