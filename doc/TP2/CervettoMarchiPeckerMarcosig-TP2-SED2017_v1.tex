...
\subsection{Autómata Celular\label{sec:AC}} 
 ...
 El primer bloque corresponde a una fila de celdas que manejarán la ubicación inicial de los productos dentro del inventario. Del trabajo práctico 1 se puede recordar que cada producto tiene una fecha de vencimiento asociada. Esta fecha será la que determine la columna en la que se apilará a cada producto. Por ejemplo: si falta menos de una semana para su vencimiento se apilará en la columna 0 (más a la derecha), si falta entre 1 y 2 semanas en la columna 1, entre 2 y 3 en la columna 2, etc. Este proceso se esquematiza en la Figura \ref{fig:AC-ingreso-de-productos}.
...
Los productos ingresan por la entrada \texttt{in} y se van desplazando hacia la izquierda a una velocidad representada por el tiempo entre ejecuciones de las reglas ($100~\textrm{ms}$).
...
\begin{enumerate}
 ...
 \item Si la respuesta que recibe la celda de parte del inventario es nula entonces hay lugar y se envía el producto por el mismo puerto por el que se realizó la consulta: \texttt{[(0,0)!0,0+send(output,(0,0)!1),0]} y se coloca un $0$ en su lugar.
 \item Si en cambio no hay lugar entonces se indica colocando un $1$ en el elemento \texttt{indicador} de la tupla: \texttt{[(0,0)!0,(0,0)!1,1]}.
\end{enumerate}

Este conjunto de reglas junto con la estructura graficada en la Figura~\ref{fig:CT-automata} definen el modelo de la cinta transportadora. Los links y la asignación de reglas se pueden ver en la siguiente definición dentro del archivo \verb|cinta.ma|.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
El acomodamiento de los productos dentro del inventario es el siguiente: los productos van formando una pila en la columna donde entran y sólo se pueden mover hacia abajo o hacia la derecha. Un producto se podrá mover a la derecha sólo si el paso del tiempo hace que el vencimiento del mismo ya pueda pertenecer a la columna de la derecha. Si la posición se encuentra ocupada, no hay movimiento.
Puede ocurrir una situación de disputa si se libera una posición que puede se tomada tanto por el bloque superior como por el bloque contiguo. Esta situación se representa en la Figura~\ref{fig:IP-disputa}. En este caso, se comparan las fechas de vencimiento de ambos productos y la ocupa el de vencimiento más próximo.
...
\begin{enumerate}
	...
	\item Esta regla es la que agenda el momento en que el producto en una celda debe avanzar hacia la derecha. En ese momento se dispara una transición que genera el reacomodamiento del producto. Para esto, simplemente escribe un 1 en la posición 3 de la tupla al momento de vencer el producto (según su columna). 
...
Este conjunto de reglas con la estructura graficada en la Figura~\ref{fig:IP-estructura} definen el modelo del inventario de productos. Los \textit{links} y la asignación de reglas se pueden ver en la siguiente definición dentro del archivo \verb|inventario.ma|:
...
Este conjunto de reglas junto con la estructura graficada en la Figura~\ref{fig:DP-estructura} definen el modelo del despacho de productos. Los \textit{links} y la asignación de reglas se pueden ver en la siguiente definición dentro del archivo \verb|despacho.ma|:
...
Para este ejemplo se utilizó una cinta transportadora de $6$ celdas. El tiempo de demora desde que la condición de una regla en las transiciones locales es válida hasta que se ejecuta se configuró en $100~\textrm{ms}$, mientras que para las reglas de las transiciones en los puertos de entrada se configuró en $1~\textrm{ms}$. Con el primer tiempo se modeliza la velocidad de avance de la cinta transportadora.

Los valores iniciales de las celdas se cargaron mediante el archivo \texttt{cinta.val} cuyo contenido se muestra a continuación:
...
Para analizar el comportamiento del autómata celular que utiliza las reglas antes mencionadas y la secuencia de eventos de entrada del archivo \texttt{cinta.ev} se ejecutó el simulador \texttt{CD++} con el parámetro \texttt{-v} de modo de tener una salida \textit{verbosa}. La salida de la prueba es la siguiente:
...
Esta salida se obtuvo del archivo \texttt{log} del modelo \textit{top} reteniendo solamente los eventos de entrada (X) y salida (Y). El primer producto que ingresa en la cinta lo hace durante la inicialización en $t = 00:00:00:000$ (mediante el archivo \texttt{cinta.val}) y por ese motivo no aparece aquí. Este primer producto tiene una fecha de vencimiento de $0.3~\textrm{s}$. En el tiempo $0$ se evalúan todas las celdas y el producto avanza una celda hacia la izquierda alcanzando la celda $(0,4)$ de la Figura~\ref{fig:CT-automata}. A los $100~\textrm{ms}$ vuelven a evaluarse las reglas y sólo se ejecutan aquellas celdas que tienen dentro de su vecindario a la celda que sufrió modificaciones en el paso anterior. En este paso el producto no avanza un casillero más hacia la izquierda pues en la comparación de la fecha de vencimiento del producto, \texttt{(0,0)!1}$=0.3~\textrm{s}$, con el rango de fechas en la columna asociada del inventario, \texttt{(0,0)!0 + time/1000}$=0.2~\textrm{s}+0.1~\textrm{s}$, el producto se queda en la celda $(0,4)$. Entonces en este paso se consulta al inventario si hay lugar disponible en la columna conectada a la salida \texttt{out2} asociada a la celda \texttt{(0,4)} enviando un $-1$. Un tiempo después, $t = 00:00:00:111$, llega la respuesta del inventario indicando con un $1$ que no hay lugar en esa columna. Entonces el producto avanza una posición hacia la izquierda en la cinta transportadora y se repite la consulta al inventario pero esta vez desde la salida \texttt{out3} asociada a la celda \texttt{(0,3)}. Al tiempo, $t = 00:00:00:221$, llega la respuesta del inventario indicando con $0$ que hay lugar disponible en la columna por la que se consultó e instantáneamente la cinta descarga el producto por la salida. Luego de un tiempo, $t = 00:05:00:000$, ingresa a la cinta transportadora un producto con fecha de vencimiento $300.4~\textrm{s}$. Este producto avanza en la cinta hasta la posición $(0,3)$, cuando \texttt{(0,0)!1}$=300.4~\textrm{s}\leq$\texttt{(0,0)!0 + time/1000}$=0.3~\textrm{s}+300.1~\textrm{s}$, consulta si hay lugar en el inventario y se descarga allí en $t = 00:06:00:000$. De igual manera se procede con el producto que ingresa a la cinta transportadora en $t = 00:10:00:000$ con valor de fecha de vencimiento $600.2~\textrm{s}$. Finalmente, el último producto en ingresar a la cinta transportadora de valor $900.5~\textrm{s}$ lo hace en $t = 00:15:00:000$. Éste avanza hasta la celda $(0,3)$, cuando \texttt{(0,0)!1}$=900.5~\textrm{s}\leq$\texttt{(0,0)!0 + time/1000}$=0.3~\textrm{s}+900.2~\textrm{s}$, y consulta al inventario si hay lugar en la columna asociada a esta celda. Ante la respuesta negativa de éste, el producto avanza una posición más en la cinta transportadora y vuelve a consultar. Esta operación se repite y en todos los casos el inventario responde que no tiene lugar para que la cinta descargue el producto, y por lo tanto el producto alcanza el final de la cinta transportadora y permanece allí.

Para tener mayor detalle de la ejecución de las reglas se puede analizar la salida del simulador en modo \textit{verboso}. De allí se puede verificar que en cada paso de simulación sólo se evalúan aquellas celdas que tienen en su vecindario a una celda que sufrió cambios en el paso de simulación anterior. De esta manera no están ejecutándose todo el tiempo todas las reglas para todas las celdas. Esto se puede observar en el siguiente extracto de la salida que corresponde al ensayo anterior (donde para facilitar la legibilidad se quitaron algunas líneas):
...
Se observa el ingreso del producto de fecha de vencimiento $300.4~\textrm{s}$ por el puerto de entrada en $t = 00:05:00:000$. Este evento modifica el campo \texttt{producto} de la tupla en la celda \texttt{(0,5)}. Dado que la demora de las reglas asociadas con entradas es de $1~\textrm{ms}$, en $ t = 00:05:00:001$ se determinan aquellas celdas que se pueden ver afectadas por el cambio en la celda \texttt{(0,5)}, que son las celdas \texttt{(0,4)} y \texttt{(0,5)}, y se evalúa cómo repercute este cambio. A partir de la ejecución de la primera y segunda regla de cada celda el producto es tomado por la celda \texttt{(0,4)} y se quita de la \texttt{(0,5)}. Las reglas locales se ejecutan cada $100~\textrm{ms}$, por lo tanto en el tiempo $t = 00:05:00:101$ se evalúan aquellas celdas que pueden verse afectadas por las celdas que sufrieron cambios en el paso anterior. Estas son las celdas \texttt{(0,3)}, \texttt{(0,4)} y \texttt{(0,5)}. En este paso el producto avanza una celda más hacia la izquierda pasando a la celda \texttt{(0,3)}. Nuevamente se esperan $100~\textrm{ms}$ para volver a evaluar las reglas y en este caso se determina que la celda \texttt{(0,3)} es la que le corresponde al producto y entonces se consulta inventario por la disponibilidad de lugar. Se puede observar que como el valor de las celdas evaluadas en $t = 00:05:00:201$ no cambió entonces no se vuelven a evaluar las reglas locales. La respuesta del inventario llega a la celda \texttt{(0,3)} en el tiempo $t = 00:06:00:000$ donde indica con $0$ que hay lugar disponible en la columna por la que se consultó. A continuación la cinta transportadora descarga el producto de fecha de vencimiento $300.4~\textrm{s}$ y coloca un $0$ en el elemento \texttt{producto} de la tupla en la celda \texttt{(0,3)}.




\subsubsection{Inventario}
Las pruebas de este módulo individual se realizaron cargando valores iniciales de las celdas con el archivo \verb|inventario.val|, en conjunto con los eventos configurados en el archivo \verb|inventario.ev| y observando la evolución de los productos en el inventario.

Por medio de la Figura~\ref{fig:IP-movimiento} se puede observar el movimiento lateral y hacia abajo de un producto dentro del inventario. Este movimiento depende de si su fecha de vencimiento cae dentro del rango de fechas comprendidas dentro de la columna en la que está ubicado o no, como se explicó anteriormente. Las celdas que no tienen valor están vacías, mientras que las que están llenas muestran las fechas de vencimiento de los producto en ellas. En concreto, se va a analizar el movimiento del producto con fecha de vencimiento $320~\textrm{s}$.

\begin{figure}[h] 
	\centering
	\subcaptionbox{t = 00:05:19:800}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_05_19_800}
		\label{fig:IP-movimiento-a}
	}
	\hfill
	\subcaptionbox{t = 00:05:19:900}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_05_19_900}
		\label{fig:IP-movimiento-b}
	}
	\subcaptionbox{t = 00:05:20:000}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_05_20_000}
		\label{fig:IP-movimiento-c}
	}\hfill
	\subcaptionbox{t = 00:05:20:100}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_05_20_100}
		\label{fig:IP-movimiento-d}
	}
...
\begin{figure}[h] 
	\centering
	\subcaptionbox{t = 00:20:00:001}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_20_00_001}
		\label{fig:IP-ensayodisputa-a}
	}
	\hfill
	\subcaptionbox{t = 00:20:00:101}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_20_00_101}
		\label{fig:IP-ensayodisputa-b}
	}
	\subcaptionbox{t = 00:20:00:201}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_20_00_201}
		\label{fig:IP-ensayodisputa-c}
	}\hfill
	\subcaptionbox{t = 00:20:00:301}{
		\centering
		\includegraphics[width=0.4\linewidth]{00_20_00_301}
		\label{fig:IP-ensayodisputa-d}
	}
...
Los valores iniciales de las celdas de este autómata se cargaron con el archivo \verb|despacho.val|. Para la prueba de este módulo también se procedió a excitarlo con un archivo de eventos \verb|despacho.ev|, que contiene los pedidos de productos y las respuestas del inventario:
...
Se tuvo especial cuidado en emular las respuestas del inventario en los momentos adecuados de acuerdo con las consultas realizadas al mismo. Consideramos que la asignación de los valores de las reglas tiene un retraso de $100~\textrm{ms}$, que modeliza la velocidad del despachante.
...
\begin{enumerate}
	...
	\item Luego de $1~\textrm{ms}$ el pedido se registra en la oficina del despachante (celda \texttt{(0,6)}).
	\item Luego de $100~\textrm{ms}$, el despachante camina hasta la primera columna del inventario (celda \texttt{(0,5)}) y hace el pedido al inventario.
	\item En el tiempo $00:01:00:103$ el inventario responde que no hay productos en esa columna.
	\item Luego de $100~\textrm{ms}$ más el despachante camina una columna más y repite el pedido.
	\item Se repite el proceso con sus respectivos tiempos hasta que el despachante llega a la columna 3 (celda \texttt{(0,2)}) donde el inventario responde con un producto con fecha de caducidad $600.2~\textrm{s}$ (recordar que el control de calidad se realiza por un modelo atómico mostrado en el TP1) en tiempo $00:01:00:412$.
	\item El producto se debe desplazar hacia la derecha a $100~\textrm{ms}$ por celda hasta salir por el puerto de salida de pedidos.
	...
	\item Finalmente el proceso se repite en el tiempo $00:02:00:000$ pero el inventario dispone de producto en la columna más cercana al despachante (celda \textrm{(0,5)}). Por lo que el recorrido del despachante es solamente de una posición.
\end{enumerate}

Para verificar el comportamiento esperado anterior, se procedió a analizar los archivos \verb|log| generados. Filtrando con el comando \verb|grep|, y conociendo la nomenclatura de los mensajes se puede analizar el comportamiento.

Para la salida del \textit{top-model} podemos observar:
...
Donde se observa que $101~\textrm{ms}$ después de cada respuesta del inventario se cursa un pedido a la columna siguiente. Por otro lado se observa que el producto sale correctamente por la salida \textit{pedidosOut} $503~\textrm{ms}$ después de haber entrado en el despacho, tal como fue previsto. Se observa coherencia también en los casos siguientes, con un tiempo de demora en la salida proporcional a la distancia recorrida por el despachante para encontrar un producto en el inventario.
...
Se comprobó la utilidad del formalismo cell-DEVS para simular el comportamiento de sistemas espaciales que implican dinámicas en varias dimensiones. 

Utilizando reglas simples por celda y replicando estas celdas se pueden lograr comportamientos emergentes o complejos. Pudimos experimentar también que a partir de la definición del vecindario de cada celda se determina qué grupo de celdas evaluar en cada paso de simulación, resultando en una ejecución más eficiente que modelando con otro formalismo.

A lo largo del desarrollo del trabajo práctico con la herramienta \verb|CD++| en su versión \textit{standalone} se encontraron algunos inconvenientes: a) cuando se incluye un bloque generador se genera un error que indica que el bloque no está registrado, b) cuando se ejecuta el simulador avanzado con el parámetro \texttt{-r} se produce una excepción y se aborta la ejecución, c) el archivo de salida generado por el simulador con la opción \texttt{-o} genera un archivo vacío, y d) no pudimos definir reglas por zonas ni utilizar algunas \textit{macros}. El primer punto nos llevó a probar todos los autómatas recurriendo a archivos de eventos. Por su parte, el no poder utilizar el comando \verb|cd++ -r| nos dificultó el \textit{debug} de las reglas para cada autómata. Cada vez que invocamos al simulador con esta opción obtuvimos el siguiente mensaje de error:

\begin{lstlisting}
Exception MException thrown!
Description: Rules 8 and 19 evaluate to TRUE and their results are different!
Thrown in: 
File: synnode.cpp - Method: evaluate - Line: 118
\end{lstlisting}

\noindent Por esta razón utilizamos el comando \verb|cd++ -v| que devuelve mucha más información y requiere de un análisis más detallado. Por último, el hecho de no poder definir reglas que se ejecuten por zonas dificultó la implementación del autómata celular correspondiente al inventario donde se tuvieron que agregar varias reglas para las filas y columnas en los bordes que tienen un comportamiento distinto a las restantes. Por este mismo motivo, en lugar de haber utilizado $3$ autómatas celulares (cinta transportadora, inventario, despacho de productos) para resolver el problema de este trabajo, podría haberse utilizado tan sólo uno, definiendo zonas cada una con sus reglas.

La posibilidad de incorporar tuplas del simulador avanzado simplifica enormemente los vecindarios a utilizar y da mayor flexibilidad.

Para analizar el resultado del autómata celular correspondiente al inventario utilizamos el visualizador online \url{http://omarhesham.com/arslab/webviewer/} en conjunto con el \textit{script} de python provisto por la cátedra y corregido para seleccionar el elemento de la tupla a visualizar. Sería interesante contar con un visualizador que permita ver más de un elemento de la tupla a la vez.

Al integrar los $3$ autómatas celulares implementados en el \texttt{topModel.ma} y ensayarlo obtuvimos el siguiente error:
\begin{lstlisting}
...
\noindent Ejecutando el simulador en modo \textit{verboso} podemos ver que la excepción se debe a un error de \textbf{rollback}:
\begin{lstlisting}
StateVars: ()
...
El desarrollo del presente trabajo, en conjunto con el primer TP, nos permitió introducirnos en el tema de simulación por eventos discretos y apropiarnos de los conceptos básicos del formalismo.
