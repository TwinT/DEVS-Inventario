...
\begin{figure}[h] 
	\centering 
	\includegraphics[width=0.8\textwidth]{INV-estructura.png} 
...
\begin{figure}[h] 
	\centering 
	\includegraphics[width=0.4\textwidth]{disputa} 
...
Estas reglas se ejecutan periódicamente para ordenar los productos en el inventario de modo de ubicar aquellos con fecha de vencimiento próximo lo más cerca posible de la salida (abajo y a la derecha). Las reglas se agrupan en pares de reglas complementarias asociadas con el movimiento de los productos hacia la derecha y hacia abajo y tienen el siguiente significado:
\begin{enumerate}
	\item Las primeras cuatro reglas, en grupos de dos, están relacionadas con el movimiento hacia abajo y hacia la derecha de un producto respectivamente. Si una celda está vacía y la celda a su izquierda tiene un producto que de acuerdo con su fecha de vencimiento debe moverse a la columna actual pero que comparando su fecha de vencimiento con la del producto en la celda que está encima de la celda vacía es mayor entonces esta última \textit{gana} la celda vacía.
	\item Las siguiente regla es un caso particular de la última fila del inventario. En este caso, un producto sólo puede desplazarse hacia la derecha o salir del inventario.
		\item Las siguientes cuatro reglas son un caso particular en el caso que se libere una celda y uno y sólo uno de los bloques (el de arriba o el  de la izquierda) exista efectivamente para disputarla.
		\item La siguiente regla es un caso particular la última fila del inventario. En este caso, el producto se desplaza hacia la derecha, pero sin disputa de un bloque superior.
	\item Las siguientes cuatro reglas correspondes a los casos particulares de la columna de la izquierda(cuyas celdas no tienen disputa de un producto contiguo) y la fila de arriba(cuyas celdas no tienen disputa de un producto de arriba) 
